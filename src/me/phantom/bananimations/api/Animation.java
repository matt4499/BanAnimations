...
public abstract class Animation {
   ...
   public Animation(String name, String versionDepend) {
      this.name = name;
      this.versionDepend = versionDepend;
   ...
   public void freeze(Player target) {
      ...
      if (!target.isOnGround()) {
         ...
         target.addPotionEffect(new PotionEffect(PotionEffectType.JUMP, 10000, 150));
         Location targetLocation = target.getLocation();
         Location teleportLocation = new Location(targetLocation.getWorld(), targetLocation.getX(), (double)targetLocation.getBlockY(), targetLocation.getZ());
   ...
   public void hook() {
      try {
         if (this.versionDepend == null || (double)Integer.parseInt(this.versionDepend.split("\\.")[1]) <= this.getPlugin().getVerMain()) {
