...
public class SpitAnimation extends Animation {
   ...
   public void playAnimation(CommandSender sender, Player target, AnimationType type, String reason) {
      ...
      for(int var9 = 0; var9 < var8; ++var9) {
         ...
         llama.teleport(llama.getLocation().setDirection(targetLocation.clone().subtract(llama.getLocation()).toVector()));
         llamas.add(llama);
      }
   ...
   private Location[] getLocationsCross(Location center) {
      ...
      Location[] cross = new Location[]{new Location(world, x - 1.5D, y, z), new Location(world, x, y, z + 1.5D), new Location(world, x, y, z - 1.5D), new Location(world, x + 1.5D, y, z)};
      return cross;
   }
}